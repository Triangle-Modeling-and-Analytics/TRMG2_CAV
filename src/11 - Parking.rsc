...
Macro "Calculate Parking Cores" (MacroOpts)
    ...
    // Add CAV parkhome trips (original CAV parkhome trip (fromhome) + parking trips (tohome) + reverse return (fromhome again) + remaining trips which use park shuttle and walk)
    cores.(auto_core) := nz(cores.(auto_core)) + 2 * nz(cores.(auto_core + "_parkhome_fromhome")) + nz(cores.(auto_core + "_parkhome_tohome"))
